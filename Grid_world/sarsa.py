...
def play_game(grid, Q, E, learning_rate, epsilon, visibility = False):
	...
	while running:
		...
		a_next = epsilon_action(max_dict(Q[s_next])[0],epsilon)

		delta = r + grid.gamma * Q[s_next][a_next] - Q[s][a]
		E[s][a] += 1

		for states in grid.states(): ##### + terminal states????
			Qsa_old = Q[s][a]
			Q[s][a] += learning_rate * delta * E[s][a]
			E[s][a] *= gamma * grid.Lambda
...
def sarsa(grid):
	...
	for n in range(n_episodes):
		...
		# set eligibility trace = 0 for all s,a
		for s in states:
			E[s] = {}
			for a in possible_actions:
				E[s][a] = 0
		...
		Q, delta = play_game(grid, Q, E, learning_rate, epsilon, visibility)
...
if __name__ == '__main__':
	...
	grid = make_grid(width, height, start, actions, rewards, step_cost, obey_prob, epsilon, gamma, learning_rate, Lambda)

	policy, V, delta = sarsa(grid)

	np.savez('sarsa_data.npz',V, policy, delta)
